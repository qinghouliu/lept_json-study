...
static void test_access_array() {
	lept_value a, e;
	size_t i, j;
	//lept_value 的变量 a 和 e，预计是一个结构体，代表一个 JSON 值。size_t i, j 用作循环计数器。
	lept_init(&a);//初始化 JSON 值 a，可能是设置必要的默认值或分配内存。

	for (j = 0; j <= 5; j += 5) {
		//j = 0，然后 j = 5。它测试了数组在这些初始容量下的行为。
		lept_set_array(&a, j);//数组 a 的容量设置为 j
		//printf("**************%d\n", j);// 0 5
		//printf("**************%d\n", lept_get_array_size(&a));
		EXPECT_EQ_SIZE_T(0, lept_get_array_size(&a));
		EXPECT_EQ_SIZE_T(j, lept_get_array_capacity(&a));
		for (i = 0; i < 10; i++) {
			//这个循环无论初始容量如何都向数组添加 10 个元素，测试动态调整大小
			lept_init(&e);
			lept_set_number(&e, i);
			lept_move(lept_pushback_array_element(&a), &e);//初始化 e，将其设置为数值 i，将 e 移动到数组 a 中（添加到末尾），然后释放 e。
			lept_free(&e);
		}

		EXPECT_EQ_SIZE_T(10, lept_get_array_size(&a));//检查数组大小是否现在为 10。
		for (i = 0; i < 10; i++)//
			EXPECT_EQ_DOUBLE((double)i, lept_get_number(lept_get_array_element(&a, i)));//验证数组中的元素是否与预期的数字 0 到 9 匹配
	}
	//printf("**************%d\n", lept_get_array_size(&a));


	lept_popback_array_element(&a);//从数组中移除最后一个元素。
	EXPECT_EQ_SIZE_T(9, lept_get_array_size(&a));//断言移除最后一个元素后数组大小现在为 9。
	for (i = 0; i < 9; i++)
		EXPECT_EQ_DOUBLE((double)i, lept_get_number(lept_get_array_element(&a, i)));
	//遍历数组，检查数组中的元素是否与预期（从0到8的数字）匹配。
	//尝试擦除不存在和存在的元素
	// 4 0  
	// 8  1 
	// 0  2
	lept_erase_array_element(&a, 4, 0);//尝试从数组中擦除索引4开始的0个元素，此操作应不改变数组。
	EXPECT_EQ_SIZE_T(9, lept_get_array_size(&a));//断言擦除操作后数组大小仍为9。
	for (i = 0; i < 9; i++)
		EXPECT_EQ_DOUBLE((double)i, lept_get_number(lept_get_array_element(&a, i)));
	      //再次验证数组内容未发生变化。
	...
	lept_erase_array_element(&a, 0, 2); //从数组的开头擦除两个元素。
	EXPECT_EQ_SIZE_T(6, lept_get_array_size(&a));//验证擦除两个元素后数组的大小是否为6。
	for (i = 0; i < 6; i++)
	...
	for (i = 0; i < 8; i++)
		EXPECT_EQ_DOUBLE((double)i, lept_get_number(lept_get_array_element(&a, i)));
	////断言数组的容量大于8，并执行收缩操作以匹配当前元素数量
	lept_set_string(&e, "Hello", 5);//向数组添加一个字符串元素 "Hello"

	lept_move(lept_pushback_array_element(&a), &e);     /* Test if element is freed */
	lept_free(&e);

	//记录当前容量，清空数组，验证数组大小为0，收缩容量也为0。
...
static void test_access_object() {
	...
	lept_init(&o);//初始化 JSON 值 o，这通常涉及设置类型和分配内存。

	for (j = 0; j <= 5; j += 5) {
		//printf("**************%d\n", j);// 0 5
		//printf("**************%d\n", lept_get_object_size(&o));
		lept_set_object(&o, j);
		EXPECT_EQ_SIZE_T(0, lept_get_object_size(&o));
		EXPECT_EQ_SIZE_T(j, lept_get_object_capacity(&o));
		//为对象 o 设置容量，并断言对象的初始大小为0且容量正确设置为 j
		for (i = 0; i < 10; i++) {
			char key[2] = "a";
			...
			lept_free(&v);
			//在此循环中，为对象 o 添加 10 个键值对，键名从 "a" 到 "j"，对应的值从 0 到 9。每次添加后都重新初始化 v。
		}
		EXPECT_EQ_SIZE_T(10, lept_get_object_size(&o));//检验对象 o 的大小是否为10。
		for (i = 0; i < 10; i++) {
			char key[] = "a";
			key[0] += i;
	...
	//	printf("number-----%f\n", lept_get_number(lept_get_object_value(&o, lept_find_object_index(&o, key, 1))));
	//}
	//从对象中删除键 "a" 的值，并确认 "a" 不再存在，对象大小变为8。
	index = lept_find_object_index(&o, "a", 1);
	EXPECT_TRUE(index != LEPT_KEY_NOT_EXIST);
	lept_remove_object_value(&o, index);
	index = lept_find_object_index(&o, "a", 1);
	EXPECT_TRUE(index == LEPT_KEY_NOT_EXIST);
	EXPECT_EQ_SIZE_T(8, lept_get_object_size(&o)); // bcdefghi
	...
	EXPECT_TRUE(lept_get_object_capacity(&o) > 8);  // 10
	lept_shrink_object(&o);                         // 8
	//printf("capacity: %d\n", lept_get_object_capacity(&o));
	...
	//向对象添加一个键 "World"，其值为字符串 "Hello"，验证添加成功且值正确。
	...
	//记录当前容量，清空对象，验证大小为0且容量未改变，然后收缩对象容量至0。
	lept_free(&o);
	//释放与对象 o 相关的所有资源，完成测试。
#endif
}
